...
<template>
    ...
    <div class="letraroxa fundobranco menu">
        ...
        <div>
            <NuxtLink 
                v-bind:class="['margin', 'persobtn', linkSelecionado == 'inicio' ? 'ativo' : '']" 
                to="/" 
                v-on:click="alteraLinkSelecionado('inicio')">
                Página inicial
            </NuxtLink>
            <NuxtLink 
                v-bind:class="['margin', 'persobtn', linkSelecionado == 'mesas' ? 'ativo' : '']" 
                to="/monte-sua-festa" 
                v-on:click="alteraLinkSelecionado('mesas')">
                Monte Sua festa
            </NuxtLink>
            <NuxtLink 
                v-bind:class="['margin', 'persobtn', linkSelecionado == 'carrinho' ? 'ativo' : '']" 
                to="/carrinho" 
                v-on:click="alteraLinkSelecionado('carrinho')">
                Carrinho
            </NuxtLink>
            <NuxtLink 
                v-bind:class="['margin', 'persobtn', linkSelecionado == 'perfil' ? 'ativo' : '']" 
                to="/perfil" 
                v-on:click="alteraLinkSelecionado('perfil')">
                {{ usuarioLogado ? 'Perfil' : 'Login' }}
            </NuxtLink>
        </div>
    </div>
</template>
